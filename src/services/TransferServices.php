...
class TransferService implements AccountTransaction
{
    ...
    {
        $amountTransfer = $this->transactionService->commitTransaction(
            $accountFrom, 
            $accountTo, 
            AccountTransaction::TRANSFER_TRANSACTION, 
            $amount, 
            $comment, 
            $dueDate, 
            $this->transactionRef);
        
        if ($amountTransfer && is_null($this->transactionRef));
        {
